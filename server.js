...
// --- LIVE SESSION ENGINE ---
// This function creates a session that is always "LIVE NOW"
const getMockSessions = () => {
    const now = new Date();
    const start = new Date(now.getTime() - (15 * 60 * 1000)); // 15 mins ago
    const end = new Date(now.getTime() + (45 * 60 * 1000));  // 45 mins from now
    
    return [{
        id: "live-session-999",
        name: "IMPORTANT: Class Connect Live",
        ...
        canJoin: true,
        isRequired: true,
        timeZone: "UTC" // Some components require this
    }];
};

// --- ROUTES ---

// 1. Coverage for all possible Class Connect endpoints
app.get(['/api/classconnect', '/api/coach/classconnect', '/api/teacher/liveclass'], (req, res) => {
    res.json(getMockSessions());
});

// 2. Coverage for Canvas Events (Fallback)
...
app.get('/api/assignments', async (req, res) => {
    ...
    try {
        ...
        const [coursesRes, plannerRes] = await Promise.all([
            axios.get(`${CANVAS_URL}/api/v1/courses?include[]=enrollments`, { headers }),
...
// Auth & Admin Posts (keep your existing logic here...)
